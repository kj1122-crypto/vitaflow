...
function AiCoach({ isPro, userName }) {
  ...
  async function send() {
    ...
    try {
      ...
      else setMessages(m => [...m, { role: 'assistant', content: 'Sorry, please try again.' }])
  ...
  return (
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
        {chips.map(q => (<button key={q} onClick={() => setInput(q)} style={{ padding: '6px 12px', borderRadius: 20, background: goldLight, border: '1px solid ' + goldBorder, fontSize: 12, color: gold, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500 }}>{q}</button>))}
      </div>
      <div style={{ background: '#f9f9f9', border: '1px solid #eee', borderRadius: 16, padding: 16, height: 300, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 12 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ maxWidth: '85%', padding: '10px 14px', borderRadius: 14, fontSize: 14, lineHeight: 1.6, alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', background: m.role === 'user' ? '#0A0A0A' : '#fff', color: m.role === 'user' ? '#fff' : '#1a1a1a', border: m.role === 'assistant' ? '1px solid #eee' : 'none' }}>
            {m.role === 'assistant' && <div style={{ fontSize: 10, color: gold, fontWeight: 600, marginBottom: 4 }}>VITACORE AI</div>}
            {m.content}
          </div>
        ))}
        {loading && <div style={{ fontSize: 13, color: '#999', fontStyle: 'italic' }}>VitaCore AI is thinking...</div>}
        <div ref={endRef} />
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder="Ask anything about your health..." style={{ flex: 1, background: '#fff', border: '1px solid #ddd', borderRadius: 12, padding: '12px 14px', color: '#1a1a1a', fontFamily: 'inherit', fontSize: 14, outline: 'none' }} />
        <button onClick={send} style={{ width: 44, height: 44, borderRadius: 12, background: '#0A0A0A', border: 'none', color: '#fff', fontSize: 18, cursor: 'pointer', flexShrink: 0 }}>up</button>
      </div>
    </div>
  )
}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', padding: 28, width: '100%', maxWidth: 480, maxHeight: '80vh', overflowY: 'auto' }}>
        ...
        <div style={{ fontSize: 13, color: '#666', lineHeight: 1.8, marginBottom: 20 }}>
          In accordance with the Personal Data Protection Act (PDPA), VitaCore requests your consent to collect and process your health data including heart rate, calories, sleep duration, steps, body weight, SpO2, GPS location during workouts, and connected device data.
          Your data will be stored securely, used only to improve your health insights, never sold to third parties, and is deletable at any time.
        </div>
        <button onClick={onAgree} style={{ width: '100%', padding: 14, borderRadius: 12, background: '#0A0A0A', border: 'none', color: '#fff', fontFamily: 'inherit', fontSize: 15, fontWeight: 600, cursor: 'pointer', marginBottom: 10 }}>I Agree and Continue</button>
...
function OnboardingGuide({ step, onClose, onNext }) {
  const steps = [
    ...
    { title: 'Earn XP and Rewards', desc: 'Complete daily quests to earn XP. Redeem for smart watches, vouchers, and exclusive prizes!', icon: 'Quest' },
    { title: 'Friends and Chat', desc: 'Add friends, send messages, and motivate each other on your health journeys together.', icon: 'Chat' },
    { title: 'Health Tracking', desc: 'Connect your smartwatch to auto-sync BPM, calories, sleep and more. View up to 1 month of history.', icon: 'Watch' },
  ]
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', padding: 28, width: '100%', maxWidth: 480 }}>
        ...
        <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginBottom: 20 }}>
          {steps.map((_, i) => (<div key={i} style={{ width: i === step ? 20 : 6, height: 6, borderRadius: 3, background: i === step ? gold : '#e0e0e0', transition: 'all 0.3s' }} />))}
        </div>
...
function ChatTab({ profile }) {
  ...
  function sendChat() {
    ...
    setChatInput('')
    setTimeout(() => { setChatMsgs(m => [...m, { role: 'them', text: 'That is awesome! Keep pushing!' }]) }, 1000)
  }
  ...
  if (activeFriend) return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid #f0f0f0', background: '#fff' }}>
        <button onClick={() => setActiveFriend(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#666' }}>back</button>
      ...
      <div style={{ display: 'flex', gap: 8, padding: 16, borderTop: '1px solid #f0f0f0', background: '#fff' }}>
        ...
        <button onClick={sendChat} style={{ width: 40, height: 40, borderRadius: 10, background: '#0A0A0A', border: 'none', color: '#fff', fontSize: 16, cursor: 'pointer', flexShrink: 0 }}>up</button>
      </div>
    </div>
  )
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ fontSize: 18, fontWeight: 600 }}>Friends and Chat</div>
      ...
      {showAddFriend && (
        <div style={{ background: goldLight, border: '1px solid ' + goldBorder, borderRadius: 14, padding: 16, marginBottom: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>Send Friend Request</div>
          <div style={{ display: 'flex', gap: 8 }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {['7D', '2W', '1M'].map((t, i) => (<button key={t} style={{ padding: '5px 10px', borderRadius: 8, background: i === 0 ? '#0A0A0A' : '#f0f0f0', border: 'none', color: i === 0 ? '#fff' : '#666', fontSize: 11, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>{t}</button>))}
        </div>
      </div>
      <div style={card}>
        ...
        <div style={{ display: 'flex', gap: 4, alignItems: 'flex-end', height: 60 }}>
          {bpmData.map((v, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{ width: '100%', background: '#E24B4A', borderRadius: '3px 3px 0 0', height: (v / maxBpm * 50) + 'px' }} />
      ...
      <div style={{ background: '#0A0A0A', borderRadius: 14, padding: 16, marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ fontSize: 32 }}>watch</div>
          <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 12 }}>
          {[{ l: 'Today BPM', v: '72 avg', c: '#E24B4A' }, { l: 'Active time', v: '1h 24m', c: '#10b981' }, { l: 'Weather', v: '31C Cloudy', c: '#2196F3' }, { l: 'Location', v: 'KL, Malaysia', c: gold }].map(m => (
...
export default function Dashboard() {
  ...
  const REWARDS = [
    { icon: 'watch', name: 'VitaCore Watch', xp: 50000, unlocked: userXp >= 50000 },
    { icon: 'scale', name: 'Smart Scale', xp: 20000, unlocked: userXp >= 20000 },
    { icon: 'food', name: 'KFC RM50', xp: 5000, unlocked: userXp >= 5000 },
    { icon: 'food', name: 'McDonalds RM50', xp: 5000, unlocked: userXp >= 5000 },
    { icon: 'run', name: 'Marathon Entry', xp: 8000, unlocked: userXp >= 8000 },
    { icon: 'music', name: 'Earbuds', xp: 15000, unlocked: userXp >= 15000 },
  ]

  if (!profile) return (
    <div style={{ background: '#fff', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 14, color: '#999' }}>Loading VitaCore...</div>
      </div>
    </div>
  )
    ...
    <div style={{ background: '#fafafa', minHeight: '100vh', fontFamily: '-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif', color: '#1a1a1a' }}>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', paddingBottom: 80 }}>
        <nav style={{ display: 'flex', padding: '10px 8px 0', position: 'sticky', top: 0, background: '#fff', zIndex: 100, borderBottom: '1px solid #f0f0f0', boxShadow: '0 1px 6px rgba(0,0,0,0.04)' }}>
          {[['home', 'Home'], ['ai', 'AI Coach'], ['health', 'History'], ['chat', 'Friends'], ['pro', 'Plans']].map(([t, label]) => (
            <button key={t} onClick={() => setTab(t)} style={nb(tab === t)}><span style={{ fontSize: 17 }}>*</span>{label}</button>
          ))}
        </nav>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <button onClick={logout} style={{ background: 'none', border: '1px solid #e0e0e0', borderRadius: 8, padding: '5px 10px', color: '#999', cursor: 'pointer', fontSize: 11, fontFamily: 'inherit' }}>Sign out</button>
                </div>
              </div>
              ...
              {[{ icon: 'salad', title: 'Grilled Salmon Bowl', sub: 'Lunch - 480 kcal - 42g protein', id: 'm1' }, { icon: 'gym', title: 'Upper Body Strength', sub: '5:30 PM - 45 min - 320 kcal', id: 'w1' }, { icon: 'moon', title: 'Quinoa Veggie Stir-fry', sub: 'Dinner - 390 kcal', id: 'm2' }].map(item => (
                <div key={item.id} style={{ ...card, display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ fontSize: 20, width: 40, height: 40, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f5f5', flexShrink: 0 }}>*</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>{item.title}</div>
                    <div style={{ fontSize: 11, color: '#999', marginTop: 2 }}>{item.sub}</div>
                  </div>
                  <div onClick={() => setChecks(c => ({ ...c, [item.id]: !c[item.id] }))} style={{ width: 26, height: 26, borderRadius: '50%', border: checks[item.id] ? 'none' : '1.5px solid #e0e0e0', background: checks[item.id] ? '#10b981' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 13, color: '#fff', flexShrink: 0 }}>{checks[item.id] ? 'ok' : ''}</div>
                </div>
              ))}
              ...
              {[{ name: 'KL Marathon 2026', date: 'Jun 15 - Earn 2,000 XP', color: '#2196F3', bg: '#EBF4FF' }, { name: 'Badminton Tournament', date: 'Jul 8 - Earn 1,500 XP', color: '#10b981', bg: '#E8F8F2' }, { name: 'Pickleball Open', date: 'Jul 22 - Earn 1,500 XP', color: '#FF6B35', bg: '#FFF0EB' }].map(c2 => (
              ...
              <div style={{ ...card, background: '#0A0A0A' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#666', lineHeight: 1.5 }}>Auto-sync BPM, calories, sleep and location</div>
                  </div>
                  <button onClick={() => { if (!pdpaAgreed) setShowPdpa(true) }} style={{ padding: '7px 12px', borderRadius: 8, background: gold, border: 'none', color: '#fff', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>Connect</button>
                </div>
          ...
          {tab === 'ai' && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, padding: 14, background: goldLight, borderRadius: 14, border: '1px solid ' + goldBorder }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>AI</div>
                <div style={{ flex: 1 }}>
          ...
          {tab === 'pro' && (
            <div>
              <div style={{ textAlign: 'center', padding: '12px 0 16px' }}>
                <div style={{ fontSize: 32, marginBottom: 6 }}>crown</div>
              ...
              <div style={card}>
                ...
                {['Basic health tracking', '3 AI messages/day', 'XP quests and leaderboard', 'Friends and chat'].map(f => <div key={f} style={{ fontSize: 12, color: '#666', marginBottom: 5, display: 'flex', gap: 7 }}><span style={{ color: '#10b981' }}>ok</span>{f}</div>)}
              </div>
              <div style={{ ...card, border: '1.5px solid #2196F3' }}>
                ...
                {['Unlimited AI coach', 'Advanced analytics', 'Workout video library', 'Smart device sync', 'Custom meal plans', 'Health history up to 1 month'].map(f => <div key={f} style={{ fontSize: 12, color: '#666', marginBottom: 5, display: 'flex', gap: 7 }}><span style={{ color: '#10b981' }}>ok</span>{f}</div>)}
              ...
              <div style={{ ...card, border: '1.5px solid #10b981' }}>
                ...
                {['All Pro Monthly features', 'Gym partner discounts', 'Priority AI coaching', 'Full nutrition plans', 'Early feature access'].map(f => <div key={f} style={{ fontSize: 12, color: '#666', marginBottom: 5, display: 'flex', gap: 7 }}><span style={{ color: '#10b981' }}>ok</span>{f}</div>)}
              ...
              <div style={{ ...card, background: '#0A0A0A', border: '1.5px solid ' + gold }}>
                ...
                {['Private 1-on-1 health coach session monthly', 'Monthly blood test analysis and consultation', 'Exclusive VitaCore Watch gifted on signup', 'VIP marathon and sports event access', 'Unlimited AI nutrition and health coaching', '24/7 health concierge support hotline', 'Annual comprehensive full body health report', 'Priority customer support and onboarding call'].map(f => (
