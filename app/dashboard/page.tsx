...
function AiCoach({ name }: { name: string }) {
  const [msgs, setMsgs] = useState<{ role: "user" | "assistant"; content: string }[]>([
    { role: "assistant", content: `Good morning ${name}! I have reviewed your health data. Your WellScore is 82 and heart rate is steady at 72 BPM. How can I help you today?` }
  ])
  ...
  return (
    <div>
      <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6, marginBottom: 12 }}>
        {chips.map(q => <button key={q} onClick={() => setInput(q)} style={{ padding: "6px 12px", borderRadius: 20, background: GL, border: `1px solid ${GB}`, fontSize: 12, color: GD, cursor: "pointer", fontWeight: 500 }}>{q}</button>)}
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Ask your AI health coach anything..." style={{ flex: 1, background: "#fff", border: "1px solid #E5E7EB", borderRadius: 12, padding: "12px 16px", color: "#1F2937", fontSize: 14, outline: "none" }} />
